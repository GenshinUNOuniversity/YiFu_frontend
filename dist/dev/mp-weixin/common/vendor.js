...
function callHook$1(hook, instance, type) {
  callWithAsyncErrorHandling(
    isArray$1(hook) ? hook.map((h3) => h3.bind(instance.proxy)) : hook.bind(instance.proxy),
    instance,
    type
  );
}
...
function setResult(result, k2, v) {
  result[k2] = v;
}
...
function createInvoker(initialValue, instance) {
  const invoker = (e3) => {
    ...
    if (instance && instance.ctx.$getTriggerEventDetail) {
      if (typeof e3.detail === "number") {
        e3.detail = instance.ctx.$getTriggerEventDetail(e3.detail);
      }
    }
    ...
    if (bubbles.includes(e3.type) && !eventSync) {
      ...
      const res = invoke();
      if (e3.type === "input" && (isArray$1(res) || isPromise(res))) {
        return;
      }
      return res;
    }
      ...
      for (var i2 in parent2) {
        ...
        try {
          ...
          if (e3 instanceof TypeError) {
            continue;
          } else if (e3 instanceof ReferenceError) {
            continue;
          }
        }
      }
